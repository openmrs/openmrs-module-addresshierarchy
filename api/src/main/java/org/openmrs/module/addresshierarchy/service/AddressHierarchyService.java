...
public interface AddressHierarchyService {
	 ...
	 * specified addressField) This method can handle restrictions based on address field values not
	 * only above but also *below* the specified level. (For instance, if the city is set to "Boston",
	 * and we ask for possible values for the "state" level, only Massachusetts should be returned)
	 * 
	 * @param address the address we are testing against
	 * @param fieldName name of the address field to look up possible values for
	 * @return a list of the names of the possible valid address values for the specified field; returns
	 *         an empty list if no matches, should return null only if error
	 */
	public List<String> getPossibleAddressValues(PersonAddress address, String fieldName);
	
	/**
	 * Given a map of address fields to address field values, returns the names of all entries that are
	 * hierarchically valid for the specified addressField. (Excluding duplicate names and ignoring any
	 * current value of the specified addressField) This method can handle restrictions based on address
	 * field values not only above but also *below* the specified level. (For instance, if the city is
	 * set to "Boston", and we ask for possible values for the "state" level, only Massachusetts should
	 * be returned)
	 * 
	 ...
	 * specified level). This method can handle restrictions based on address field values not only
	 * above but also *below* the specified level. (For instance, if the city is set to "Boston", and we
	 * ask for possible values for the "state" level, only Massachusetts should be returned)
	 * 
	 * @param address
	 * @param level
	 * @return a list of possible valid address hierarchy entries; returns an empty list if no matches,
	 *         should return null only if error
	 */
	 ...
	 * at the highest level to the entry at the lowest level in the tree. For example, the full address
	 * for the Beacon Hill neighborhood in the city of Boston might be: "United
	 ...
	 * list of full addresses, represented as a pipe-delimited string of address hierarchy entry names,
	 * ordered from the entry at the highest level to the entry at the lowest level in the tree. For
	 * example, the full address for the Beacon Hill neighborhood in the city of Boston might be:
	 * "United States|Massachusetts|Suffolk County|Boston|Beacon Hill" Note that if the Name Phonetics
	 * module has been installed, and the global property addresshierarchy.soundexProcessor has been set
	 * to the name of a recognized soundex processor, this method will perform a soundex search as
	 ...
	 * that search string Note that if the Name Phonetics module has been installed, and the global
	 * property addresshierarchy.soundexProcessor has been set to the name of a recognized soundex
	 * processor, this method will perform a soundex search as opposed to a straight word search If no
	 * level is specified, this method operates as if a searchAddress(String) call
	 *
	 * @param searchString the search string to search for
	...
	/**
	 * Returns all address hierarchy entries at the given level that have the specified name (name match
	 * is case-insensitive)
	 *
	 * @param level
	 * @param name
	...
	/**
	 * Saves a block of address hierarchy entries within a single transaction (This should be used with
	 * care since the save interceptors may not be used properly. This method is mainly used to speed up
	 * the performance of importing a hierarchy from a file)
	 * 
	 * @param entries
	 */
	@Authorized({ AddressHierarchyConstants.PRIV_MANAGE_ADDRESS_HIERARCHY })
	 ...
	 * 
	 * @param includeUnmapped specifies whether or not to include hierarchy levels that aren't mapped to
	 *            an underlying address field
	 * @return the ordered list of address hierarchy levels
	 */
	 ...
	 * and stores this in a local cache for use by the searchAddresses(String) method The map values are
	 * full addresses represented as a pipe-delimited string of address hierarchy entry names, ordered
	 * from the entry at the highest level to the entry at the lowest level in the tree. For example,
	 * the full address for the Beacon Hill neighborhood in the city of Boston might be: "United
	 * States|Massachusetts|Suffolk County|Boston|Beacon Hill" In the standard implementation, the keys
	 * are the same as the values. However, if the Name Phonetics module has been installed, and the
	 * addresshierarchy.soundexProcessor global property has been configured, the keys will be the same
	 * pipe-delimited string, but with each entry name transformed via the specified soundex processor
	 * The searchAddresses method compares the input string against the keys, and returns the values of
	 * any matches Need to make sure we synchronize to avoid having multiple threads trying to
	 * initialize it at the same time, or one using it before it is initialized (Note that the one thing
	 * this won't prevent against is it being re-initialized while another thread is accessing it) NOTE:
	 * this method ONLY initializes the full address cache if it has not yet been initialized-- if you
	 * want to RE-initialized the full address cache, first call resetFullAddressCache and then this
	 * method
	 */
	...
	/**
	 * Given a person, updates all the AddressToEntryMaps for all non-voided PersonAddresses associated
	 * with that person
	 * 
	 * @param person
	 */
	...
	/**
	 * I've renamed the following methods to make them a little more clear, but kept the old methods for
