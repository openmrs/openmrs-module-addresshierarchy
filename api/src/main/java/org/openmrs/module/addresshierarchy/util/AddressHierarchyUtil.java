...
public class AddressHierarchyUtil {
	...
	public static final String getAddressFieldValue(PersonAddress address, AddressField field) {
		try {
			Method getter = PersonAddress.class
	...
	public static final void setAddressFieldValue(PersonAddress address, AddressField field, String value) {
		try {
			Method setter = PersonAddress.class.getMethod(
			    "set" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1), String.class);
			setter.invoke(address, value);
		}
		catch (Exception e) {
			throw new AddressHierarchyModuleException("Unable to set address field " + field.getName() + " on PersonAddress",
			        e);
		}
	}
	...
	/**
	 * Tests whether the first addresses hierarchy entry is the descendant of the second--i.e., can you
	 * reach the second entry by travelling up the tree from the first hierarchy entry
	 */
	 ...
	 * with this patient (Pulled this out the AddressHierarchyService so that the entire operation
	 * doesn't happen as part of a single transaction)
	 */
	 ...
	 * library is only included in OpenMRS 1.9; I didn't want to import the class into address hierarchy
	 * module directly for fear of having classloading issues when running on OpenMRS 1.9; once Address
	 * Hierarchy switches to only support OpenMRS 1.9+, we can import lang3 and delete this method)
	 * <p>
